      ...
      <div className='userProfile border border-dark m-5'>
        {profile.map((user) => (
          <div  key={user.id} className='comman'>
               ...
               <h5 style={{fontSize:16}}>{user.username}</h5>
               <h6 className='ms-auto text-primary' style={{fontSize:12}} >switch</h6>
          </div>
        ))}
          ...
          <div  key={suggestion.id} className='comman'>
               ...
               <h5 style={{fontSize:16}}>{suggestion.username}<br/>
                 <p style={{fontSize:8,color:'#00000096',margin:0}} >{suggestion.reason}</p>
               </h5>
               <h6 className='ms-auto text-primary' style={{fontSize:12,cursor:'pointer'}} onClick={()=>toggleFollow(suggestion.id)}>
